...
В репозитории Vagrantfile и окружение ansible разворачивают LDAP-сервер на дистрибутиве FreeIPA, а также две клиентские машины.<br>
После установки на сервере добавляется пользователь *otus-user*.<br>
Далее разворачиваются две клиентских ВМ, в которых: создаются уникальные пользователи (otus-user1 и otus-user2), генерируются ключи ssh-rsa и добавляются на сервер.<br>
